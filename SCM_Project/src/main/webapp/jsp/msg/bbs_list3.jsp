	...
	<table align=center width=640 cellpadding="0" cellspacing="0">
		...
		<c:if test="${listcount > 0 }">
			...
			<tr align="center" valign="middle">
				<td colspan="4" >받은 메세지</td>
				<td align=right><font size=2>받은 갯수 :
						${listcount }</font></td>
			</tr>
			...
			<tr align=center height=20>
				<td colspan=7>
					...
					<%--} --%> <c:if test="${page <= 1 }">
						...
						<a href="bbs_list3.msg?page=${page-1}&text=${text}">◀</a>&nbsp;
				...
				if(a==nowpage){ // 현재 페이지 --%> <%-- [<%=a %>] --%> <%--}else{ --%> <%-- <a href="./BoardList.bo?page=<%=a %>">[<%=a %>]</a>&nbsp; --%>
					<%--} --%> <%--} --%> <c:forEach var="a" begin="${startpage}"
						...
						<c:if test="${a != page }">
							<a href="bbs_list3.msg?page=${a}&text=${text}" >${a}</a>&nbsp;
				</c:if>
		...
		<c:if test="${listcount == 0 }">
			<tr align="center" valign="middle">
				<td colspan="4">받은 메세지</td>
				<td align=right><font size=2>받은 메세지가 없습니다.</font></td>
			</tr>
		</c:if>
