...
<form name="profile_form" method="post" enctype="multipart/form-data" action="prf_modify.prf">
	<div>
		...
		<!-- 아이디, 이름, 패스워드 가져가는 부분 -->
		<input type="hidden" name="PROFILE_ID" value="${appbean.PROFILE_ID }">
		<input type="hidden" name="PROFILE_USER" value="${appbean.PROFILE_USER }">
		<div id="profile-pic">
			<div id="pic">
				<c:if test="${appbean.PROFILE_PIC != null }">
					
				<img src="./upload/prf/${appbean.PROFILE_PIC }" width="200px" height="273px">
 				</c:if>	
				<c:if test="${appbean.PROFILE_PIC == null }">
					프로필사진을 설정해 주세요
				</c:if>	
